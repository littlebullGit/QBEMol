...
from numpy import diag_indices, einsum, float64, trace, zeros_like
...
if TYPE_CHECKING:
    from quemb.molbe.vqe_solver import VQE_ArgsUser
...
def _run_vqe_fragment(
    fobj: Frags | pFrags,
    vqe_args: "VQE_ArgsUser",
    be_energy: float | None,
    use_cumulant: bool,
    eeval: bool,
) -> tuple:
    """
    Run VQE on a single fragment. Designed for use with ThreadPoolExecutor.

    Parameters
    ----------
    fobj : Frags or pFrags
        Fragment object with heff already updated
    vqe_args : VQE_ArgsUser
        VQE configuration parameters
    be_energy : float or None
        Current BE energy for adaptive convergence
    use_cumulant : bool
        Whether to use cumulant approximation
    eeval : bool
        Whether to evaluate energy

    Returns
    -------
    tuple
        (frag_name, rdm1_mo, rdm2, energy_components or None)
    """
    from quemb.molbe.vqe_solver import VQE_ArgsUser, solve_vqe

    # Perform SCF calculation first (sets up fobj._mf and fobj.mo_coeffs)
    # This mirrors the sequential be_func behavior
    assert fobj.fock is not None and fobj.heff is not None
    fobj.scf()

    # solve_vqe needs the fragment's _mf object
    # It regenerates FCIDUMP internally with current heff
    ...
    e_f = None
    if eeval:
        # Apply cumulant correction if needed
        rdm2_for_energy = rdm2s.copy()
        if use_cumulant:
        ...
        e_f = get_frag_energy(
            ...
            h1=fobj.h1,
            rdm1=rdm1_tmp,
            rdm2s=rdm2_for_energy,
            dname=fobj.dname,
            veff0=fobj.veff0,
            veff=None,  # use_cumulant=True means veff=None
            use_cumulant=use_cumulant,
            eri_file=fobj.eri_file,
        )

    return (fobj.dname, rdm1_tmp, rdm2s, e_f)
...
def be_func_parallel(
    ...
    # VQE uses ThreadPoolExecutor (threads share memory, no pickling needed)
    # Other solvers use ProcessPool (multiprocessing)
    if solver == "VQE":
        return _be_func_parallel_vqe(
            Fobjs=Fobjs,
            Nocc=Nocc,
...
def _be_func_parallel_vqe(
    ...
    """
    Parallel VQE fragment solving using ThreadPoolExecutor.

    VQE fragments are independent within a BE iteration and can be solved
    in parallel. We use threads instead of processes because:
    1. Fragment objects contain complex state that's hard to pickle
    2. VQE is largely I/O bound (Qiskit simulator calls)
    3. Threads share memory, avoiding serialization overhead

    Parameters
    ----------
    Fobjs : list
        Fragment objects with heff already updated
    Nocc : int
        Number of occupied orbitals
    solver_args : VQE_ArgsUser
        VQE configuration
    use_cumulant : bool
        Whether to use cumulant approximation
    eeval : bool
        Whether to evaluate energy
    return_vec : bool
        Whether to return error vector
    nprocs : int
    ...
    -------
    tuple or float
        Energy or (error_norm, error_vec, energy) depending on return_vec
    """
    from quemb.molbe.vqe_solver import VQE_ArgsUser

    assert isinstance(solver_args, VQE_ArgsUser), "VQE requires VQE_ArgsUser"

    # Get current BE energy for adaptive convergence (use first fragment's estimate)
    be_energy = None  # Will be set by VQE state tracking internally

    print(
        f"[VQE Parallel] Running {len(Fobjs)} fragments with {nprocs} threads",
        flush=True,
    )

    # Run VQE on all fragments in parallel using threads
    results_by_name: dict[str, tuple] = {}

    with ThreadPoolExecutor(max_workers=nprocs) as executor:
        # Submit all fragment jobs
        future_to_frag = {
            executor.submit(
                _run_vqe_fragment,
                fobj,
        ...
        for future in as_completed(future_to_frag):
            frag_name = future_to_frag[future]
            try:
                result = future.result()
                results_by_name[result[0]] = result
                print(f"[VQE Parallel] Completed fragment {frag_name}", flush=True)
            except Exception as e:
                print(f"[VQE Parallel] ERROR in fragment {frag_name}: {e}", flush=True)
                raise

    # Update fragment objects with results (must match order)
    total_e = [0.0, 0.0, 0.0]

    for fobj in Fobjs:
        dname, rdm1_tmp, rdm2s, e_f = results_by_name[fobj.dname]

        # Store MO-basis RDM
        fobj.rdm1__ = rdm1_tmp.copy()
        vqe_trace = trace(rdm1_tmp)
        print(
            f"DEBUG: VQE parallel RDM trace for {dname}: {vqe_trace:.6f} "
            f"(expected: {2 * fobj.nsocc})",
            flush=True,
        )

        # Transform RDM to AO basis for density matching
        assert fobj.mo_coeffs is not None
        fobj._rdm1 = multi_dot((fobj.mo_coeffs, rdm1_tmp, fobj.mo_coeffs.T)) * 0.5

        # Store RDM2 for energy
        if eeval:
            # Recompute cumulant-corrected RDM2 for storage
            rdm2_store = rdm2s.copy()
            if use_cumulant:
                assert fobj.nsocc is not None
                hf_dm = zeros_like(rdm1_tmp)
                hf_dm[diag_indices(fobj.nsocc)] += 2.0
            ...
            for callback in _solver_result_callbacks:
                try:
                    callback(fobj, "VQE", e_f, rdm1_tmp, rdm2_store, use_cumulant)
                except Exception as e:
                    print(f"  Warning: solver callback failed for {fobj.dname}: {e}")

        fobj.mo_coeffs = fobj.mo_coeffs  # Already set, just ensuring consistency

    if eeval and not return_vec:
        Ecorr = sum(total_e)
        return (Ecorr, total_e)

    ernorm, ervec = solve_error(Fobjs, Nocc, only_chem=True)  # VQE uses only_chem=True

    if return_vec:
        return (ernorm, ervec, [sum(total_e), total_e])
